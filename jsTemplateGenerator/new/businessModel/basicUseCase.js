...
const basicModel = require('../../../datasource/db/public/basicModel');
      ...
      return knex.transaction((trx)=> {
        Promise.all([
            ...
            Promise.map(businessModel.ForeignKey, (table)=> {
              return models.foreign[`${table.Table}Model`].addData(data[`${table.Table}Data`], trx)
            }),
            Promise.map(businessModel.MappingKey, (table)=> {
              return models.mapping[`${table.MiddleTable}Model`].addData(data[`${table.MiddleTable}Data`], trx)
            })
          ])
          .then(trx.commit)
          .catch(trx.rollback);
